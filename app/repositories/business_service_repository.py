...
class BusinessServiceRepository(BaseRepository[BusinessServiceModel]):
    ...
    def _mapping(self, record: dict) -> dict:
        ...
        json_fields = ["business", "commodity"]
    ...
    def _build_query(self) -> Select:
        ...
        return (
            select(
                ...
                self.model.business_id.label("business_id"),
                self.model.commodity_id.label("commodity_id"),
                self.model.name.label("name"),
                self.model.latitude.label("latitude"),
                self.model.longitude.label("longitude"),
                self.model.visitor_type_id.label("visitor_type_id"),
    ...
    ) -> Tuple[List[BusinessServiceModel], int]:
        filters = filters or []
        sort = sort or []
    ...
    @override
    async def find_by_id(self, id: str, relationships: Optional[List[str]] = None) -> Optional[BusinessServiceModel]:
    ...
    @override
    async def create(self, data: Dict[str, Any]) -> BusinessServiceModel:
        data = await super().create(data)
        return await self.find_by_id(data.id)

    @override
    async def update(self, id: str, data: Dict[str, Any], refresh: bool = True) -> BusinessServiceModel:
